...
public static class VillageMapBuilder
{
    ...
    {
        // 이 생성기는 삭제된 무료팩을 참조한다. 실행하면 씬을 새로 만들면서
        // 지금까지 손으로 다듬은 VillageScene이 통째로 사라지므로 막아둔다.
        if (!AssetDatabase.IsValidFolder(FreeRoot.TrimEnd('/')))
        {
            EditorUtility.DisplayDialog("맵 생성기 사용 불가",
                "무료팩이 없어 이 생성기는 동작하지 않습니다.\n" +
                "실행하면 현재 VillageScene이 지워지므로 중단했습니다.", "확인");
            return;
        }
